...
class Pipeline:
    ...
    def _build_github_collector(self) -> Any:
        ...
        return cls(
            ...
            max_fetch_files=gh_cfg.get("max_fetch_files", 500),
            slow_mode=gh_cfg.get("slow_mode", False),
        )
    ...
    def _build_learn_collector(self) -> Any:
        ...
        internal_url = (
            learn_cfg.get("knowledge_service_url")
            or os.getenv("LEARN_KNOWLEDGE_SERVICE_URL", "")
        )
        internal_scope = (
            learn_cfg.get("knowledge_service_scope")
            or os.getenv("LEARN_KNOWLEDGE_SERVICE_SCOPE", "")
        )
        allow_fallback = str(
            learn_cfg.get("use_public_fallback", os.getenv("USE_PUBLIC_LEARN_FALLBACK", "false"))
        ).lower() in {"1", "true", "yes", "on"}

        if internal_url:
            if not internal_scope:
                if allow_fallback:
                    print("Internal Learn scope missing — falling back to public Learn API")
                else:
                    raise ValueError("Internal Learn configured but knowledge_service_scope is missing")
            else:
                internal_cls = COLLECTOR_REGISTRY.get("learn_internal")
                if not internal_cls:
                    return None
                return internal_cls(
                    ...
                    search_path=learn_cfg.get("knowledge_search_path", "/api/search"),
                )
            # falls through to public learn when fallback is enabled
